...
# The saved models predict 60 minutes ahead.
MODEL_FORECAST_HORIZON = 60  
# Our target: predict the full next trading day (e.g., 390 minutes from 9:30 to 16:00)
TARGET_HORIZON = 390  
WINDOW_SIZE = 60       # Use the last 60 minutes of historical data as input
...
for model_path in model_files:
    # Extract the symbol from the filename, e.g., "trained_model_META.pth"
    ...
    # Load processed CSV data (assumed normalized)
    try:
    ...
    while total_predicted < TARGET_HORIZON:
        with torch.no_grad():
            batch_pred_norm = model(current_chain.unsqueeze(0)).cpu().numpy().flatten()  # shape: (60,)
        ...
        batch_pred_original = scaler.inverse_transform(placeholder)[:, 3]   # shape: (60,)
        ...
        for i in range(len(batch_pred_norm)):
            ...
            last_volume = scaler.inverse_transform(last_chain_row)[0, 4]  # index 4 for volume
            row_dict = {
